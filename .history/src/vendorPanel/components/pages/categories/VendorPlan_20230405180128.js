...
const VendorCategories = () => {
  ...
  const addCategory = async (e) => {
    e.preventDefault();
    const url = BaseUrl() + "/category/add/sellercategory";
  ...
  const editCategory = async (e) => {
    e.preventDefault();
    const url = BaseUrl() + `/category/edit/sellercategory/${edit._id}`;
    const fd = new FormData();
    fd.append("myField", image);
    fd.append("category", name);
    try {
    ...
    <>
      <section>
        <div className="pb-4 sticky top-0  w-full flex justify-between items-center bg-white">
          <span className="tracking-widest text-white font-semibold uppercase md:py-2 px-3 md:px-4 py-1 rounded-full bg-[rgb(240,72,88)]">
            All Dishes
          </span>
          <button
            onClick={() => {
        ...
        >
          <div className="bg-slate-100 overflow-y-auto  lg:w-3/6  md:w-4/6 w-5/6 mx-auto  rounded-lg">
            <div className="flex sticky top-0 py-3 px-5 bg-slate-100 justify-between">
              <span className=" font-semibold text-[rgb(240,72,88)] rounded-Full ">
                Add Categories
              </span>
